...
const doc = (style, body) => `<html><head><style>${style}</style></head><body>${body}</body></html>`;

describe('gdocHtmlToMarkdown', () => {
  it('turns a paragraph document into headings, dividers and list items', () => {
    const html = doc('.c1{font-weight:700}.c2{font-weight:400}', [
    ...
    expect(gdocHtmlToMarkdown(html)).toBe('### Matchup A\n\n- Card X: +2\n\n---\n\n- Card Y: -2\n\n- Link: [list](https://e.test/l)\n');
  });

  it('turns a table document into a GFM table', () => {
    const html = doc('', '<table><tr><td><p><span>Matchup</span></p></td><td><p><span>Plan</span></p></td></tr><tr><td><p><span>A</span></p></td><td><p><span>Go | fast</span></p><p><span>then slow</span></p></td></tr></table>');
    expect(gdocHtmlToMarkdown(html)).toBe('| Matchup | Plan |\n| --- | --- |\n| A | Go \\| fast then slow |\n');
  });
});
