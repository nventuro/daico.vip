...
export default function GuideChapterPage() {
  ...
  const chapter = index === -1 || !guide ? undefined : guideChapters[index];
  ...
  return (
    <EntryPage entry={chapter} loading={loading} missing="Capítulo no encontrado.">
      {(chapter) => (
        <article className="flex flex-col gap-4">
          <p className="text-sm text-muted">{guide?.title}</p>
          <Heading className="text-3xl">{chapter.title}</Heading>
