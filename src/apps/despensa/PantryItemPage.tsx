...
export default function PantryItemPage() {
  const { items, loading, error, save, setExpiry, mark, unmark, remove } = usePantry();
      ...
      {(item) => (
        <article key={item.id} className="flex flex-col gap-4">
          ...
          <FormField label="Vence" group>
            <MonthPicker
              value={item.expires_on && yearMonthOf(item.expires_on)}
              onChange={(yearMonth) => void setExpiry(item.id, yearMonth)}
              today={today}
            />
          </FormField>
