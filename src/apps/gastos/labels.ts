...
function number(cents: number, withCents: boolean): string {
  return (Math.abs(cents) / 100).toLocaleString('es-AR', {
...
/** Pesos in thousands, tight, for a row with no room: "$1.235k", or with a
 *  decimal under COMPACT_AMOUNT_DECIMAL_BELOW_CENTS: "$45,7k". */
export function formatArsCompact(cents: number): string {
  const decimals = Math.abs(cents) < COMPACT_AMOUNT_DECIMAL_BELOW_CENTS ? 1 : 0;
  const thousands = (Math.abs(cents) / 100_000).toLocaleString('es-AR', {
    minimumFractionDigits: decimals,
    maximumFractionDigits: decimals,
  });
  return `${cents < 0 ? '−' : ''}$${thousands}k`;
}
