...
export interface TripKindShape {
  ...
  /** Whether it goes from one place to another, on some transport. */
  route: boolean;
}
...
export const TRIP_KIND_SHAPES: Record<TripKind, TripKindShape> = {
  todo: { icon: IconChecklist, ticked: true, starts: 'day', ends: 'none', route: false },
  ticket: { icon: IconPlane, ticked: false, starts: 'day-time', ends: 'day-time', route: true },
  lodging: { icon: IconBed, ticked: false, starts: 'day', ends: 'day', route: false },
  booking: { icon: IconTicket, ticked: false, starts: 'day-time', ends: 'none', route: false },
  place: { icon: IconMapPin, ticked: false, starts: 'none', ends: 'none', route: false },
};
...
/** What stands before a pasaje, by what it travels on. */
export const TRIP_TRANSPORT_ICONS: Record<TripTransport, TablerIcon> = {
  flight: IconPlane,
  train: IconTrain,
  bus: IconBus,
};

/** What a pasaje travels on until its page says otherwise: the one the
 *  household books most. */
export const TRIP_TRANSPORT_DEFAULT: TripTransport = 'flight';

/** What stands before a staged boarding pass under review: no class of row,
 *  but the files a pasaje is boarded with. */
export const BOARDING_PASS_ICON: TablerIcon = IconQrcode;

/** Whether a pasaje is a flight: the one transport whose places are airports,
 *  and whose boarding pass only exists once it is checked in for. */
